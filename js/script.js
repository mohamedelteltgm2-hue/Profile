...
if (contactForm) {
  contactForm.addEventListener('submit', (e) => {
    ...
    // Construct URLs based on device
    ...
    submitBtn.innerHTML = isMobile ? '<i class="fas fa-external-link-alt"></i> Opening Mail App…' : '<i class="fas fa-external-link-alt"></i> Opening Gmail…';

    setTimeout(() => {
      if (isMobile) {
        // Construct mailto: URL for mobile (triggers Gmail/system app)
        ...
        // Construct Gmail Compose URL for desktop
        const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=${recipient}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`;
...
// Open mailto: links in Gmail on desktop, native app on mobile
document.addEventListener('click', (e) => {
  const mailtoLink = e.target.closest('a[href^="mailto:"]');
  if (mailtoLink) {
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);

    // Only intercept on desktop to open Gmail Web
    if (!isMobile) {
      e.preventDefault();
    ...
    // On mobile, let the default mailto: behavior run (opens Gmail App/System Mail)
  }
});
...
/* =========================================
   SAFETY LAYER (Blur Protection)
   ========================================= */

(function initSafetyLayer() {
  // 1. Create and Inject the Blur Overlay
  const safetyOverlay = document.createElement('div');
  safetyOverlay.className = 'safety-blur-overlay';
  safetyOverlay.id = 'safetyOverlay';
  safetyOverlay.innerHTML = `
    <div class="safety-message">
      <i class="fas fa-shield-alt" style="font-size: 5rem; margin: 0;"></i>
    </div>
  `;
  ...
  const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  if (isMobile) {
    console.log("Safety Layer: Mobile protection active");
  }

  // 6. Developer Tools Detection (Phase 2 Addon)
  ...
  const checkDevTools = () => {
    ...
    if (widthDiff || heightDiff) {
      if (!isMobile) { // Mobile browsers often have weird inner/outer ratios
        showSafety();
        devToolsOpen = true;
      }
    } else if (devToolsOpen) {
      // If they were open and now they aren't (and we aren't blurred for other reasons)
      // Only hide if the window has focus
  ...
  // 7. Frame-Busting (Clickjacking Protection)
  if (window.self !== window.top) {
    window.top.location = window.self.location;
  }

  // 8. Image Guard Wrapper Injection
  // We wrap the thumbs in a protective layer after the cards are rendered
  const injectImageGuards = () => {
    document.querySelectorAll('.project-thumb:not(.guarded)').forEach(thumb => {
      const guard = document.createElement('div');
      guard.className = 'image-guard-overlay';
      thumb.appendChild(guard);
      thumb.classList.add('guarded');
    });
  };

  setInterval(injectImageGuards, 2000);
